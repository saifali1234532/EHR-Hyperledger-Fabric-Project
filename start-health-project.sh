...
echo "🚀 Starting E-Health Blockchain Project..."

# Fix Docker socket permissions gracefully
sudo chmod 666 /var/run/docker.sock || true

# Dynamically target the fabric-samples directory accurately
if [ -d "./organizations" ] && [ -f "./network.sh" ]; then
    # Running directly inside test-network
    TEST_NETWORK_DIR=$(pwd)
    FABRIC_SAMPLE_DIR=$(dirname "$TEST_NETWORK_DIR")
    PROJECT_DIR=$(dirname "$FABRIC_SAMPLE_DIR")
    ...
    echo "❌ Error: Could not automatically locate the test-network directory."
    echo "Please run this script from either the repository root or the test-network folder."
    exit 1
fi
